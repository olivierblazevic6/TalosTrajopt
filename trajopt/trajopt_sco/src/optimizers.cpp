...
/* 
   ...
   improve_ratio_threshold = 0.6;
  min_trust_box_size = 0.7;
  min_approx_improve = 1e-2;
  ...
  trust_box_size = 0.7;
  */
  improve_ratio_threshold= 0.55      ;// minimum ratio true_improve/approx_improve to accept step. [Research Paper] TrueImprove/ModelImprove > c
  min_trust_box_size= 1e-2           ;// if trust region gets any smaller, exit and report convergence. [Research Paper] xtol
  min_approx_improve= 1e-2           ;// if model improves less than this, exit and report convergence
  min_approx_improve_frac= -INFINITY ;// if model improves less than this, exit and report convergence
  ...
  cnt_tolerance= 1e-2                ;// after convergence of penalty subproblem, if constraint violation is less than this, we're done. [Research Paper] ctol
  ...
  max_time= INFINITY                 ;// not yet implemented
  merit_error_coeff= 10              ;// initial penalty coefficient. [Research Paper] mu_0
  trust_box_size= 0.5               ;// current size of trust region (component-wise). [Research Paper] s
}
...
ROS_INFO("iteration %i", iter);
        ...
        {
          ROS_ERROR("approximate merit function got worse (%.3e). "
